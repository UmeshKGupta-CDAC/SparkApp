...
object EmpMaxSalary {
  def main(args: Array[String]): Unit = {
    ...
    val emp_salary_list = emp_data_without_header
      .map { x => x.split(',') }
      .map { x => x(5).toDouble }
    ...
    val max_salary = emp_salary_list
      .distinct
      .sortBy(x => x.toDouble, false, 1)
    print(max_salary.take(1).foreach(println))


    // Find minimum salary
    val min_salary = emp_salary_list
      .distinct
      .sortBy(x => x.toDouble, true, 1)
    print(min_salary.take(1).foreach(println))

    // Find Second highest salary
    val second_highest_salary = max_salary
      .zipWithIndex()
      .filter(index => index._2 == 1)
      .map(_._1)
    print(second_highest_salary.foreach(println))


    // Find second min salary
    val second_min_salary = min_salary
      .zipWithIndex()
      .filter(index => index._2 == 1)
      .map(_._1)
    ...
    val salaryWithEmployeeName = emp_data_without_header
      .map { x => x.split(',') }
      .map { x => (x(5).toDouble, x(1)) }

    val maxSalaryEmployee = salaryWithEmployeeName
      .groupByKey
